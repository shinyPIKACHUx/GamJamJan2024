...
var bulletInstance
@onready var armRightSocket = $Sockets/ArmRight
var RightRay
var RightBullet
@onready var armLeftSocket = $Sockets/ArmLeft
var LeftRay
var LeftBullet
...
func set_rays_and_bullets():
	LeftRay = armLeftSocket.get_node("RayCast3D")
	LeftBullet = armLeftSocket.bulletScene
	RightRay = armRightSocket.get_node("RayCast3D")
	RightBullet = armRightSocket.bulletScene

func fire_bullets():
	if LeftRay:
		bulletInstance = LeftBullet
	else:
		bulletInstance = bullet.instantiate()
	bulletInstance.position = LeftRay.global_position
	bulletInstance.transform.basis = LeftRay.global_transform.basis
	get_parent().add_child(bulletInstance)
	
	if RightRay:
		bulletInstance = RightBullet
	else:
		bulletInstance = bullet.instantiate()
	bulletInstance.position = RightRay.global_position
	bulletInstance.transform.basis = RightRay.global_transform.basis
	get_parent().add_child(bulletInstance)
